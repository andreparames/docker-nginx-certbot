...
parse_domains() {
    ...
    for conf_file in /etc/nginx/conf.d/*.conf*; do
        sed -n -e 's&^\s*ssl_certificate_key\s*\/etc/letsencrypt/live/\(.*\)/privkey.pem;\s*$&\1&p' $conf_file | xargs echo | tr ' ' ','
    done
}

parse_wildcard_domains() {
    # For each configuration file in /etc/nginx/conf.d/*.conf*
    for conf_file in /etc/nginx/conf.d/*.conf*; do
        sed -n -e 's&^\s*ssl_certificate_key\s*\/etc/letsencrypt/live/\(.*\)/privkey.pem; #wild\s*$&\1&p' $conf_file | xargs echo | tr ' ' ','
    done
}
...
get_wildcard_certificate() {
    echo "Getting certificate for domain $1 on behalf of user $2"
    certbot certonly --agree-tos --keep -n --text --email $2 --server \
        https://acme-v02.api.letsencrypt.org/directory \
        --authenticator certbot-dns-cpanel:cpanel \
        --certbot-dns-cpanel:cpanel-credentials /etc/cpanel_credentials.ini \
        -d \*.$1 -d $1  --debug
}
